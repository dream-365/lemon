...
namespace Lemon.Core.Discover
{
    public class DiscoverObject
    {
        ...
        public IBuildIndex Builder { get; set; }

        private HttpClient _httpClient;

        public DiscoverObject(DiscoverSetting setting)
        {
        ...
        public async Task ForEachAsync(Action<BsonDocument> forEach)
        {
            for (int i = 0; i < _length; i++)
            {
                ...
                using (var sr = new StreamReader(stream, _encoding))
                {
                    ...
                    Builder.Build(text).ToList().ForEach(forEach);
                }
            }
        }
    }
